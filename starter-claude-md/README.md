...
- `CLAUDE.starter.md` — the template (EN), ready to copy. Four sections:
  ...
  in this toolkit), work order for bugfixes (piece 02), hooks installed in
  the repo (pieces 01 and 04). Every rule ends with the `[NN]` tag of the
  piece it comes from.
- `check_starter.py` — deterministic checker (no LLM): `untagged-rule` (every
  ...
  from the closed set `{01,02,03,04,07,08,09}` (the pieces this toolkit
  ...
  `<ADJUST:`).
- `fixture/` — a clean case, eight planted violations, the raw template as an
  **intentional** red ("forgotten copy" vs. "adjusted copy") and one refusal
  case; runner with regression locks.
...
Nine cases, each red locked to its named reason (locks in the runner).
Classes: rule with no tag or with a tag outside the set, doctrine
...
allowlist masking, a never-exemptable exemption refused.
